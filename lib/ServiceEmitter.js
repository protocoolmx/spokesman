...
class ServiceEmitter extends EventEmitter {
  ...
  constructor (opts) {
     ...
     * @type {Boolean}
     */
    this._runImmediately = opts.runImmediately;
     ...
     * @type {Boolean}
     */
    this._autoRequest = opts.autoRequest;
  ...
  /**
   * Validates that options provided are of correct type, if not then assign default value.
   * 
   * @private
   * @param {Object} opts - Contructor options to validate.
   */
  _validateOpts (opts) {

    let defaults = {
      delay: { type: 'number', value: 1000 },
      runImmediately: { type: 'boolean', value: false },
      autoRequest: { type: 'boolean', value: true }
    };

    for (let key in defaults) {
      if (opts[key] !== undefined && typeof opts[key] === defaults[key].type) {
  ...
  /**
   * For a ServiceEmitter instance only listening to "data" and "error" is allowed, when someone
   * calls {@link this.removeAllListeners()} only this two events are removed. "newListener" and
   * "removeListener" event listeners untouched.
   */
  removeAllListeners() {
   ...
   * implementation. This callback also emits validated data on 'data' event
   * by default. If error is provided then it is emitted on 'error' event and
   * return.
   *
   * Note: This callback is optional, and if you decide to omit it then you
   * should emit data received on 'data' event to let know listeners when there is
   * new data.
   *
